...
function Add_New_Forum() {
  ...
  const { addMainThread, loading } = useHotelForumStore();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Failed to add thread: ${error.message || "Unknown error"}`);
    }
  };
        ...
        <div className="flex items-center justify-between border-b border-gray-300 p-4">
          <div className="flex items-center gap-4">
            ...
            <span className="text-lg font-medium">
              Add New Forum Thread (Staff Only)
            </span>
          </div>
          <Button
            type="submit"
          ...
          <ScrollArea className="flex-1 border border-gray-300 bg-white h-[calc(100vh-120px)] rounded-md mx-6 mt-6 mb-6 p-4">
            <Card className="space-y-2 rounded-none border-none bg-none shadow-none">
              <CardHeader className="p-2">
                ...
                <CardDescription>
                  Provide the title and content for the new forum thread.
                </CardDescription>
              </CardHeader>
