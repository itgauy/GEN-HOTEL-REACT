...
// THIS IS ALL HOTEL RESERVATION QUEUE

const useGuestReserveStore = create(
  persist(
    (set, get) => ({
      reservations: [],
      loading: false,
      error: null,
      userId: null, // Store user ID for revalidation
      ...
      revalidateReservations: async () => {
        const { userId, fetchReservationsId } = get();
        if (userId) {
          await fetchReservationsId(userId);
        } else {
          set({ 
            error: 'No user ID available for revalidation', 
            loading: false 
          });
        }
      },
